...
std::pair<WifiStatus, sp<IWifiApIface>> WifiChip::createApIfaceInternal() {
    ...
    // M: We use ap0 for AP interface
    LOG(ERROR) << "We use ap0 for AP interface";
    std::string ifname = "ap0";
...
WifiStatus WifiChip::handleChipConfiguration(
    ...
    } else if (mode_id == kV2ChipModeId) {
        // M: Set concurrent mode for STA+SAP
        success = mode_controller_.lock()->setConcurrentMode(true);
        // M: Need to set at least one of STA and SAP mode for driver.
        success = mode_controller_.lock()->changeFirmwareMode(IfaceType::STA);
    }
    if (!success) {
        return createWifiStatus(WifiStatusCode::ERROR_UNKNOWN);
...
void WifiChip::populateModes() {
    ...
    if (feature_flags_.lock()->isDualInterfaceSupported()) {
        LOG(ERROR) << "Dual Interface Supported";
        ...
        // M: We support STA+SAP+P2P
        const IWifiChip::ChipIfaceCombination chip_iface_combination_1 = {
            {chip_iface_combination_limit_1, chip_iface_combination_limit_2, chip_iface_combination_limit_3}};
